...
export type ComponentRender<Props extends Record<string, any>> = (
  this: void,
...
export function defineComponent(optionsOrRender: any, config?: Config): string {
  config = extend(
    {
  ...
  let render: ComponentRender<Record<string, any>>
  let options: Options
  ...
  const originDetached =
    options.lifetimes[ComponentLifecycle.DETACHED] ||
